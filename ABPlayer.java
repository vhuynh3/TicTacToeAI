...
public class ABPlayer extends Player {
    public ABPlayer(String mark, int type) {
        super(mark, type);
    }
    ...
    public int play(Board board) {
        ...
        for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {
            if (board.board[i].equals(" ")){ //If space is empty
                ...
                curVal = minimax(board, 0, (type + 1) % 2,-100000,100000); //calculate the value of the board
    ...
    public int minimax(Board board, int depth, int minOrMax, int alpha, int beta) {
        ...
        if (minOrMax == MAX_PLAY) { //maximizing player
            ...
            for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {  //go through entire board
                if (board.board[i].equals(" ")) { //if space is empty
                    ...
                    bestVal = Math.max(bestVal, minimax(board, depth + 1, MIN_PLAY,alpha,beta)); 
            ...
            for (int i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) { //go through entire board
                if (board.board[i].equals(" ")) { //if space is empty
                    ...
                    bestVal = Math.min(bestVal, minimax(board, depth + 1, MAX_PLAY,alpha,beta)); 
